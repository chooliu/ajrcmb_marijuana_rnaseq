...
make_plots_and_tables_rtPCR <- function(gene_symbol, make_images = T) {
  ensg_id <- annotations_by_gene %>%
    filter(external_gene_name == gene_symbol) %>% .$ensembl_gene_id
  rtpcr_target <-
  ...
  fig_target_1 <-
    ggboxplotWrapper(rtpcr_target, "Group", "rtPCR_val") +
    ylab(expression("RT-PCR"~2^{Delta*Ct})) +
  ...
  fig_target_5 <-
    ...
    annotate(geom = "text", label =
               cor(rtpcr_target$RNAseq_val, rtpcr_target$rtPCR_val, use = "complete")) +
  ...
  ggsave(filename = paste0("./FiguresTables/rtPCR/", gene_symbol, ".png"),
         dpi = 300, width = 7.5, height = 2)
}
...
ggplot(data = rtPCR_data, aes(x = TBP, y = TBP_RNAseq)) +
  geom_smooth(method = 'lm', alpha = 0.5, color = "#d73027", se = F, size = 1) +
  annotate(geom = "text", label = cor(rtPCR_data$TBP, rtPCR_data$TBP_RNAseq,
                                      use = "complete")) +
