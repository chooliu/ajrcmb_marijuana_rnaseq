# ==============================================================================
# 21_celltype_sensitivity_pt2.R
# run the edgeR models with cell-type
# (very similar code to script 19)
# ==============================================================================




# add RUV from script 20 and re-run edgeR for cell-type analysis
# ------------------------------------------------------------------------------
...
plot_grid(
  ...
  theme_classic() +
  scale_x_continuous(expression("MvC"~-log[10]~P~", With Cell Type"),
  ...
  annotate("text", x = 4, y = 13.5,
           label = paste0("Pearson r = ",
                          cor(sensitivity_results_MvC_all$NegLogP.x,
                              sensitivity_results_MvC_all$NegLogP.y, use = "complete") %>%
                            format(digits = 2))),
  ...
  ggplot(data = sensitivity_results_MvT_all,
    ...
    geom_abline(intercept = 0, slope = 1) +
    theme_classic() +
    scale_x_continuous(expression("MvT"~-log[10]~P~", With Cell Type"),
                       limits = c(0, 14), expand = c(0, 0)) +
    scale_y_continuous(expression("MvT"~-log[10]~P~", Primary Analysis"),
                       limits = c(0, 14), expand = c(0, 0)) +
    annotate("text", x = 4, y = 13.5,
             label = paste0("Pearson r = ",
                            cor(sensitivity_results_MvT_all$NegLogP.x,
                                sensitivity_results_MvT_all$NegLogP.y, use = "complete") %>%
                              format(digits = 2))),
  ...
  ggplot(data = sensitivity_results_TvC_all,
    ...
    geom_abline(intercept = 0, slope = 1) +
    theme_classic() +
    scale_x_continuous(expression("TvC"~-log[10]~P~", With Cell Type"),
                       limits = c(0, 14), expand = c(0, 0)) +
    scale_y_continuous(expression("TvC"~-log[10]~P~", Primary Analysis"),
                       limits = c(0, 14), expand = c(0, 0)) +
    annotate("text", x = 4, y = 13.5,
             label = paste0("Pearson r = ",
                            cor(sensitivity_results_TvC_all$NegLogP.x,
                                sensitivity_results_TvC_all$NegLogP.y, use = "complete") %>%
                              format(digits = 2))),

nrow = 1)

ggsave("./FiguresTables/FigS4A.png", width = 9, height = 3)
system('open "./FiguresTables/FigS4A.png"')
...
plot_grid(
  ...
  ggplot(data = sensitivity_results_MvC_all,
    ...
    scale_x_continuous(expression("MvC"~logFC~", With Cell Type"),
                       limits = c(-10, 10), expand = c(0, 0)) +
    scale_y_continuous(expression("MvC"~logFC~", Primary Analysis"),
                       limits = c(-10, 10), expand = c(0, 0)) +
    annotate("text", x = -3.5, y = 9.5,
             label = paste0("Pearson r = ",
                            cor(sensitivity_results_MvC_all$logFC.x,
                                sensitivity_results_MvC_all$logFC.y, use = "complete") %>%
                              format(digits = 2))),
  ...
  ggplot(data = sensitivity_results_MvT_all,
    ...
    scale_color_manual(values = c("#7a7a7a", "#67001f")) +
    scale_shape_manual(values = c(1, 19)) +
    scale_x_continuous(expression(" MvT"~logFC~", With Cell Type"),
                       limits = c(-10, 10), expand = c(0, 0)) +
    scale_y_continuous(expression(" MvT"~logFC~", Primary Analysis"),
                       limits = c(-10, 10), expand = c(0, 0)) +
    annotate("text", x = -3.5, y = 9.5,
             label = paste0("Pearson r = ",
                            cor(sensitivity_results_MvT_all$logFC.x,
                                sensitivity_results_MvT_all$logFC.y, use = "complete") %>%
                              format(digits = 2))),
  ...
  ggplot(data = sensitivity_results_TvC_all,
    ...
    scale_color_manual(values = c("#7a7a7a", "#67001f")) +
    scale_shape_manual(values = c(1, 19)) +
    scale_x_continuous(expression("TvC"~logFC~", With Cell Type"),
                       limits = c(-10, 10), expand = c(0, 0)) +
    scale_y_continuous(expression("TvC"~logFC~", Primary Analysis"),
                       limits = c(-10, 10), expand = c(0, 0)) +
    annotate("text", x = -3.5, y = 9.5,
             label = paste0("Pearson r = ",
                            cor(sensitivity_results_TvC_all$logFC.x,
                                sensitivity_results_TvC_all$logFC.y, use = "complete") %>%
                              format(digits = 2))),
  nrow = 1)

ggsave("./FiguresTables/FigS4B.png", width = 9, height = 3)
system('open "./FiguresTables/FigS4B.png"')
